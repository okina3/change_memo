      ...
      <section class="text-gray-600 border border-gray-400 rounded-lg overflow-hidden">
         ...
         <div class="p-3 bg-white">
            ...
            <form action="{{ route('user.bait.update') }}" method="POST" class="max-w-lg">
               ...
               <label class="block mb-2">エサ名</label>
               <input name="name" type="text" value="{{ old('name', $bait->name) }}" class="input w-full mb-3" required>

               <div class="flex gap-2">
                  <button type="submit" class="btn bg-violet-700 hover:bg-violet-500">更新</button>
                  <a href="{{ route('user.masters.index', ['tab' => 'baits']) }}" class="btn bg-gray-400">キャンセル</a>
               </div>
            </form>
         </div>
      </section>
